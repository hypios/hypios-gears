module type INTERFACE = sig
    val render : string -> Xhtmltypes.div_content XHTML.M.elt list Lwt.t
  end
...
module Wiki_types = struct
    ...
    type wikibox = wikibox_arg int32_t
...
let parse_table_cell_attribs attribs =
  ...
  and at7 =
    ...
    with Not_found | Failure _ -> None
  and _at8 = parse_valign_attrib attribs
  and _at9 = parse_align_attrib attribs
  and _at10 = parse_scope_attrib attribs in
...
let make_href _sp addr fragment =
(*  let sp = bi.bi_sp in *)
(*
...
let default_builder :
  ...
  { Lexer.chars = (fun s -> Lwt.return [XHTML.M.pcdata s]) ;
    strong_elem = strong_elem;
    em_elem = em_elem;
    monospace_elem = monospace_elem;
    underlined_elem = underlined_elem;
...
let render s =
  Lexer.from_string () default_builder s
